...
- **神经网络引擎与 DRAM 功耗。** 核心行下方那条堆叠电源轨,显示芯片的瓦数实际去了哪里 ——
  CPU、GPU、ANE、DRAM,读自 IOReport 的能耗模型,不是估算出来的。
...
设置菜单里还有**刷新间隔**(1–5 秒)、**显示全部传感器**(约 200 个 PMU 键的完整转储)、
...
|---|---|---|
| *无 —— 正常字色* | 平稳 | 全部处于正常区间 |
| **琥珀** | 偏高 | CPU/GPU ≥ 75 °C · SSD ≥ 55 °C · 内存压力偏高 · 功耗达到芯片包络的 45 % |
| **珊瑚红** | 临界 | CPU/GPU ≥ 92 °C · SSD ≥ 68 °C · 内存紧张 · 功耗超出包络 · 电池 > 42 °C |
